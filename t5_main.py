...
def main(args):
    ...
    plt.xlabel("Pretraining checkpoint")
    plt.ylabel(R"$cos(\theta_{t}, \theta_{t-1}$")
    plt.title("Aggregate parameter direction")
    plt.savefig(f"images/t5-dir.png")
    ...
    plt.xlabel("Pretraining checkpoint")
    plt.ylabel(R"$\mathrm{cos}(\theta_{t}, \theta_{t-1}$)")
    plt.title("Parameter direction by layer")
    plt.legend()
    plt.savefig(f"images/t5-dir-by-layer.png")
