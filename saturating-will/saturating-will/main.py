...
trainloader = data.DataLoader(mnist_train, batch_size=256, shuffle=True)
testloader = data.DataLoader(mnist_test, batch_size=256, shuffle=False)
...
def train_model(
    ...
    for e in range(epochs):
        for i, (data, targets) in enumerate(trainloader):
            ...
            loss = criterion(output, labelperm[targets].cuda())
            loss.backward()
            optimizer.step()
            if i % 20 == 0:
                print(
...
def eval_model(model, testloader, labelperm=standard_perm):
    ...
    with torch.no_grad():
        ...
        for i, (data, targets) in enumerate(testloader):
            predictions = model(data.cuda()).argmax(dim=1, keepdim=True)
            acc1 = predictions.eq(labelperm[targets].cuda().view_as(predictions)).sum().item()
            correct += acc1
